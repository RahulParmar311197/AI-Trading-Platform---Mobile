...
def test_stale_partial_after_cancel_does_not_reopen_order(tmp_path):
    ...
    svc._save_recovered(req, cancelled, 'CANCELLED')
    stale = dict(cancelled); stale['status'] = 'PARTIALLY_FILLED'; stale['filled_quantity'] = 3
    recovered = svc._save_recovered(req, stale, 'STALE_PARTIAL')
...
def test_stale_open_after_rejection_does_not_reopen_order(tmp_path):
    ...
    svc._save_recovered(req, rejected, 'REJECTED')
    stale = dict(rejected); stale['status'] = 'NEW'; stale['filled_quantity'] = 0
    recovered = svc._save_recovered(req, stale, 'STALE_NEW')
...
def test_partial_fill_then_cancel_preserves_only_actual_fill(tmp_path):
    ...
    svc._save_recovered(req, broker.get_order(first.broker_order_id), 'PARTIAL')
    broker.cancel_order(first.broker_order_id)
    cancelled = svc._save_recovered(req, broker.get_order(first.broker_order_id), 'CANCELLED')
