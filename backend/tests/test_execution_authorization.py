...
def test_bound_order_without_account_context_is_rejected_even_if_another_account_is_reconciled(tmp_path):
    store = SafetyStateStore(str(tmp_path / 'safety.json'))
...
def test_account_scoped_reconciliation_authorizes_matching_account(tmp_path):
    store = SafetyStateStore(str(tmp_path / 'safety.json'))
    store.save(SafetyState(reconciliation_by_account={
        '7': {'broker_snapshot_fingerprint': 'A', 'reconciliation_generation': 1},
        '8': {'broker_snapshot_fingerprint': 'B', 'reconciliation_generation': 2},
    }))
    ...
    request = type('Request', (), {'broker_account_id': 8, 'client_order_id': 'order-8'})()
    snapshot = RiskSnapshot(broker_ready=True, broker_snapshot_fingerprint='B')
...
def test_account_scoped_reconciliation_rejects_other_account_snapshot(tmp_path):
    store = SafetyStateStore(str(tmp_path / 'safety.json'))
    store.save(SafetyState(reconciliation_by_account={
        '7': {'broker_snapshot_fingerprint': 'A', 'reconciliation_generation': 1},
        '8': {'broker_snapshot_fingerprint': 'B', 'reconciliation_generation': 2},
