...
class MultiAccountStartupRecovery:
    ...
    def __init__(
        ...
        context_attestor: BrokerContextAttestor | None,
    ) -> None:
        self.router = router
    ...
    def _orders_for_account(lifecycle: OrderLifecycle, account_id: str) -> list[dict[str, Any]]:
        return [
            ...
            if str(record.broker_account_id or "") == account_id
        ]
    ...
    def _positions_for_account(lifecycle: OrderLifecycle, account_id: str) -> list[dict[str, Any]]:
        return [
            ...
            if str(getattr(position, "broker_account_id", "") or "") == account_id
        ]
    ...
    def run(self, lifecycle: OrderLifecycle, accounts: list[BrokerAccount]) -> MultiAccountRecoveryResult:
        ...
        # Restore the authoritative local state exactly once. Multi-account recovery then
        # partitions that state by its durable broker_account_id instead of reloading it per account.
        ...
        if ambiguous_orders or ambiguous_positions:
            return MultiAccountRecoveryResult(
                False,
                (),
                "MULTI_ACCOUNT_STATE_UNSCOPED: every persisted order and position must have a broker account id",
            )
        ...
        for account in accounts:
            ...
            try:
                ...
                coordinator = ReconciliationCoordinator(
                    ...
                    context_attestor=self.context_attestor,
                    generation=self.router._next_reconciliation_generation(selected),
                )
                result = coordinator.reconcile(
                    internal_orders=self._orders_for_account(lifecycle, account_id),
                    internal_positions=self._positions_for_account(lifecycle, account_id),
        ...
        # Only clear after every account has independently produced a verified result.
        try:
            for reconciliation, context in verified:
                self.safety_store.clear(reconciliation, active_context=context)
        except Exception as exc:
