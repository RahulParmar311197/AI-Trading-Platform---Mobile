...
@dataclass(frozen=True)
class LiquidityPool:
    index:int; kind:str; price:float; tolerance:float
...
def liquidity_pools(candles:list[Candle],lookback:int=30,tolerance_bps:float=5.0)->list[LiquidityPool]:
    sw=swings(candles); pools=[]; tol=max(1e-12,tolerance_bps/10000)
    for i,x in enumerate(sw):
        for y in sw[:i]:
            if x.kind!=y.kind:continue
            if abs(x.price-y.price)/max(abs(x.price),1e-12)<=tol:
...
def liquidity_sweeps(candles:list[Candle],pools:list[LiquidityPool])->list[dict]:
    out=[]
    for p in pools:
        if p.index>=len(candles)-1:continue
...
def structure(candles:list[Candle])->dict:
    ...
    for seq in (highs,lows):
        for n,x in enumerate(seq):
            if n==0:continue
            labels.append({'index':x.index,'type':('HH' if x.price>seq[n-1].price else 'LH') if x.kind=='HIGH' else ('HL' if x.price>seq[n-1].price else 'LL'),'price':x.price})
    ...
    if len(highs)>=2 and len(lows)>=2:
        ...
        prior='BULLISH' if any(x['type'] in ('HH','HL') for x in labels[:-2]) else 'BEARISH' if labels else None
        if bos and prior and bos!=prior:choch=bos
    pools=liquidity_pools(candles); sweeps=liquidity_sweeps(candles,pools)
    return {'bias':bias,'bos':bos,'choch':choch,'swings':[asdict(x) for x in sw],'structure_labels':labels,'fvg':[asdict(x) for x in fair_value_gaps(candles)],'liquidity_pools':[asdict(x) for x in pools],'liquidity_sweeps':sweeps}
