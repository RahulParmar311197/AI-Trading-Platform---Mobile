...
class ExecutionAlertStore:
    ...
    def record(self, severity: str, reason_codes: tuple[str, ...], fingerprint: str) -> ExecutionAlertRecord:
        ...
        with sqlite3.connect(self.path) as conn:
            cursor = conn.execute("INSERT INTO execution_alerts(created_at,severity,reason_codes,fingerprint,status) VALUES (?,?,?,?, 'OPEN')", (created_at, severity, encoded, fingerprint))
            conn.commit()
    ...
    def acknowledge(self, alert_id: int) -> ExecutionAlertRecord:
        return self._transition(alert_id, "ACKNOWLEDGED")

    def resolve(self, alert_id: int) -> ExecutionAlertRecord:
        return self._transition(alert_id, "RESOLVED")
    ...
    def _transition(self, alert_id: int, target: str) -> ExecutionAlertRecord:
        ...
        with sqlite3.connect(self.path) as conn:
            ...
            if row is None:
                raise KeyError(f"execution alert {alert_id} not found")
            current = str(row[5] or "OPEN")
            if target == "ACKNOWLEDGED" and current != "OPEN":
                raise ValueError(f"cannot acknowledge alert in {current} state")
            if target == "RESOLVED" and current == "RESOLVED":
                return self._record_from_row(row)
            if target == "RESOLVED":
                conn.execute("UPDATE execution_alerts SET status='RESOLVED', resolved_at=? WHERE id=?", (now, alert_id))
            else:
                conn.execute("UPDATE execution_alerts SET status='ACKNOWLEDGED', acknowledged_at=? WHERE id=?", (now, alert_id))
            conn.commit()
            updated = conn.execute("SELECT id,created_at,severity,reason_codes,fingerprint,status,acknowledged_at,resolved_at FROM execution_alerts WHERE id=?", (alert_id,)).fetchone()
            return self._record_from_row(updated)

    def recent(self, limit: int = 50) -> list[ExecutionAlertRecord]:
        safe_limit = max(1, min(int(limit), 200))
        with sqlite3.connect(self.path) as conn:
            rows = conn.execute("SELECT id,created_at,severity,reason_codes,fingerprint,status,acknowledged_at,resolved_at FROM execution_alerts ORDER BY id DESC LIMIT ?", (safe_limit,)).fetchall()
