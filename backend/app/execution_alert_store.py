...
class ExecutionAlertStore:
    ...
    def _emit(self, alert: ExecutionAlertRecord, event_type: str) -> None:
        if self.event_publisher is not None:
            self.event_publisher.publish_once(alert, event_type)

    def record(self, severity: str, reason_codes: tuple[str, ...], fingerprint: str) -> ExecutionAlertRecord:
        created_at = datetime.now(timezone.utc).isoformat()
        with sqlite3.connect(self.path) as conn:
            cursor = conn.execute("INSERT INTO execution_alerts(created_at,severity,reason_codes,fingerprint,status) VALUES (?,?,?,?, 'OPEN')", (created_at, severity, ",".join(reason_codes), fingerprint)); conn.commit()
            alert = ExecutionAlertRecord(int(cursor.lastrowid), created_at, severity, reason_codes, fingerprint)
        self._emit(alert, "CREATED")
        return alert
    ...
    def _transition(self, alert_id: int, target: str) -> ExecutionAlertRecord:
        ...
        with sqlite3.connect(self.path) as conn:
            ...
            conn.commit()
            alert = self._record_from_row(conn.execute("SELECT id,created_at,severity,reason_codes,fingerprint,status,acknowledged_at,resolved_at FROM execution_alerts WHERE id=?", (alert_id,)).fetchone())
        self._emit(alert, target)
        return alert
    ...
    def query(self, *, limit: int = 50, offset: int = 0, severity: str | None = None, status: str | None = None, reason_code: str | None = None, created_after: datetime | None = None, created_before: datetime | None = None) -> ExecutionAlertPage:
        limit = max(1, min(int(limit), 200)); offset = max(0, int(offset)); clauses: list[str] = []; params: list[object] = []
        ...
        with sqlite3.connect(self.path) as conn:
            total = int(conn.execute("SELECT COUNT(*) FROM execution_alerts" + where, params).fetchone()[0]); rows = conn.execute("SELECT id,created_at,severity,reason_codes,fingerprint,status,acknowledged_at,resolved_at FROM execution_alerts" + where + " ORDER BY id DESC LIMIT ? OFFSET ?", params + [limit, offset]).fetchall()
