...
class ExecutionAlertOutboxWorker:
    ...
    def _due_events(self, limit: int) -> list[int]:
        ...
        with sqlite3.connect(self.path) as conn:
            rows = conn.execute("""SELECT e.id FROM execution_alert_events e LEFT JOIN execution_alert_deliveries d ON d.event_id=e.id WHERE d.delivered_at IS NULL AND COALESCE(d.attempts,0) < ? AND (d.next_attempt_at IS NULL OR d.next_attempt_at <= ?) ORDER BY e.id ASC LIMIT ?""", (self.max_attempts, now, max(1, min(limit, 100)))).fetchall()
    ...
    def run_once(self, limit: int = 25) -> list:
        ...
        for event_id in self._due_events(limit):
            ...
            if not result.delivered:
                ...
                with sqlite3.connect(self.path) as conn:
                    conn.execute("UPDATE execution_alert_deliveries SET next_attempt_at=? WHERE event_id=? AND delivered_at IS NULL", (retry_at, event_id))
                    conn.commit()
        return results
