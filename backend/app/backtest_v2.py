...
def run_backtest_v2(candles: list[Candle], starting_equity: float=100000.0, risk_percent: float=1.0, limits: RiskLimits|None=None) -> dict:
    ...
    portfolio=PaperPortfolio(starting_equity); broker=PaperBroker(); trades=[]; rejected=0; equity_curve=[]
    for i in range(20,len(candles)):
        bar=candles[i]
        closed=portfolio.process_bar({bar.symbol:bar.close})
        for c in closed:
            trades.append(BacktestV2Trade(c.symbol, portfolio.positions.get(c.symbol, None).side if c.symbol in portfolio.positions else 'UNKNOWN', 0.0, c.exit_price, c.realized_pnl, i, i, c.reason))
        ...
        fill=execute_paper(risk=risk,broker=broker); portfolio.apply_fill(order,fill)
    return {'starting_equity':starting_equity,'ending_equity':portfolio.mark({c.symbol:c.close for c in candles[-1:]} )['equity'],'realized_pnl':portfolio.realized_pnl,'open_positions':len(portfolio.positions),'risk_rejected':rejected,'equity_curve':equity_curve,'trade_journal':[t.__dict__ for t in trades]}
