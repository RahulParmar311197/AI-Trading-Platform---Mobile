...
def run_backtest_v2(candles:list[Candle], starting_equity:float=100000.0, risk_percent:float=1.0, limits:RiskLimits|None=None, mtf_timeframe:str|None=None, require_mtf_alignment:bool=False, session_policy:SessionPolicy|None=None, trailing_policy:TrailingPolicy|None=None, partial_exit_policy:PartialExitPolicy|None=None)->dict:
    ...
    histories=defaultdict(list); portfolio=PaperPortfolio(starting_equity); broker=PaperBroker(); trades=[]; rejected=0; mtf_rejected=0; session_rejected=0; partial_rejected=0; equity_curve=[]; open_entries={}; current_day:date|None=None; day_start_equity=starting_equity; daily_realized=0.0
    for event in events:
        ...
        if position and partial_exit_policy and not position.partial_taken:
            ...
            if qty>0:
                c=portfolio.partial_close(symbol,event.close,qty,partial_exit_policy.move_stop_to_breakeven); daily_realized+=c.realized_pnl
                trades.append(BacktestV2Trade(c.symbol,c.side,c.entry_price,c.exit_price,c.quantity,c.realized_pnl,open_entries[symbol]['entry_bar'],len(histories[symbol])-1,c.reason))
        closed=portfolio.process_ohlc_bar({symbol:event})
        for c in closed:
            daily_realized += c.realized_pnl; meta=open_entries.pop(c.symbol,None)
