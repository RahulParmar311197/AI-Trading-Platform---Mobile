...
def run_backtest_v2(candles:list[Candle], starting_equity:float=100000.0, risk_percent:float=1.0, limits:RiskLimits|None=None, mtf_timeframe:str|None=None, require_mtf_alignment:bool=False)->dict:
    ...
    histories=defaultdict(list); portfolio=PaperPortfolio(starting_equity); broker=PaperBroker(); trades=[]; rejected=0; mtf_rejected=0; equity_curve=[]; open_entries={}
    for event in events:
    ...
    return {'starting_equity':starting_equity,'ending_equity':portfolio.mark(final_prices)['equity'],'realized_pnl':portfolio.realized_pnl,'open_positions':len(portfolio.positions),'risk_rejected':rejected,'mtf_rejected':mtf_rejected,'equity_curve':equity_curve,'trade_journal':[t.__dict__ for t in trades]}
