...
def score(candles: list[Candle]) -> dict:
    if not candles:
        return {"score": 0, "bias": "NEUTRAL", "reasons": []}
    ...
    if ict["fvg"]: reasons.append(f"{len(ict['fvg'])} FVG zones")
    bias = "BULLISH" if points >= 2 else "BEARISH" if points <= -2 else "NEUTRAL"
    return {"score": max(-5, min(5, points)), "bias": bias, "reasons": reasons, "atr": a, "ict": ict}
