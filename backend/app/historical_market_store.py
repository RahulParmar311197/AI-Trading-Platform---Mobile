...
class HistoricalMarketStore:
    ...
    def upsert(self,candles:list[Candle]):
        ...
        rows=[(c.symbol.upper(),c.timeframe,c.timestamp.isoformat(),c.open,c.high,c.low,c.close,c.volume) for c in candles]
    ...
    def query(self,symbol:str,timeframe:str,start:datetime|None=None,end:datetime|None=None,limit:int=5000):
        ...
        if start: sql+=" AND timestamp>=?"; args.append(start.isoformat())
        if end: sql+=" AND timestamp<=?"; args.append(end.isoformat())
        sql+=" ORDER BY timestamp ASC LIMIT ?"; args.append(max(1,min(limit,100000)))
        with sqlite3.connect(self.db_path) as db: rows=db.execute(sql,args).fetchall()
    ...
    def count(self,symbol:str,timeframe:str):
        with sqlite3.connect(self.db_path) as db:return db.execute("SELECT COUNT(*) FROM candles WHERE symbol=? AND timeframe=?",(symbol.upper(),timeframe)).fetchone()[0]
