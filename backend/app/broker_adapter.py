...
def normalize_broker_update(raw: BrokerOrderUpdate | dict[str, Any], *, expected: BrokerOrderRequest | None = None) -> BrokerOrderUpdate:
    ...
    if expected is not None:
        ...
        if not math.isfinite(expected_quantity) or expected_quantity <= 0: raise ValueError("invalid requested quantity")
        if abs(quantity - expected_quantity) > 1e-9: raise ValueError("broker quantity does not match request")
