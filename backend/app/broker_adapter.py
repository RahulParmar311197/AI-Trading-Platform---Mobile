...
def normalize_broker_update(
    raw: BrokerOrderUpdate | dict[str, Any], *, expected: BrokerOrderRequest | None = None
) -> BrokerOrderUpdate:
    ...
    aliases = {
        "OPEN": "NEW", "PENDING": "NEW", "COMPLETE": "FILLED", "EXECUTED": "FILLED",
        "CANCELED": "CANCELLED", "CANCEL": "CANCELLED", "FAILED": "REJECTED",
    }
    ...
    if quantity is not None and filled is not None and filled > quantity + 1e-9:
        raise ValueError("broker filled quantity exceeds order quantity")
    client_id = str(data.get("client_order_id") or data.get("tag")).strip() if data.get("client_order_id") is not None or data.get("tag") is not None else None
    ...
    if expected is not None:
        if client_id is None or client_id != expected.client_order_id:
            raise ValueError("broker client_order_id does not match request")
        if symbol is None or symbol != expected.symbol.upper():
            raise ValueError("broker symbol does not match request")
        if side is None or side != expected.side.upper():
            raise ValueError("broker side does not match request")
        if quantity is None:
            raise ValueError("broker response missing requested quantity")
        if expected.broker_account_id is not None:
            if broker_account_id is None or broker_account_id != expected.broker_account_id:
                raise ValueError("broker account does not match request")
        if expected.broker_route is not None:
            if broker_route is None or broker_route != expected.broker_route:
                raise ValueError("broker route does not match request")
        if expected.broker_route_generation is not None:
            if broker_route_generation is None or broker_route_generation != expected.broker_route_generation:
                raise ValueError("broker route generation does not match request")
    if price is not None and price <= 0:
        raise ValueError("broker price must be positive")
    if average is not None and average <= 0:
        raise ValueError("broker average price must be positive")
    if status == "NEW" and filled is not None and abs(filled) > 1e-9:
        raise ValueError("NEW broker status requires zero filled quantity")
    if status == "PARTIALLY_FILLED" and (quantity is None or filled is None or not (0 < filled < quantity)):
        raise ValueError("PARTIALLY_FILLED broker status requires 0 < filled < quantity")
    if status == "FILLED" and (quantity is None or filled is None or abs(filled - quantity) > 1e-9):
        raise ValueError("FILLED broker status requires filled quantity equal to quantity")
    if status == "REJECTED" and filled is not None and abs(filled) > 1e-9:
        raise ValueError("REJECTED broker status requires zero filled quantity")
    if status in {"PARTIALLY_FILLED", "FILLED"} and (filled is None or filled <= 0):
    ...
    return BrokerOrderUpdate(
        order_id=order_id, status=status, client_order_id=client_id, symbol=symbol, side=side,
        quantity=quantity, filled_quantity=filled, price=price, average_price=average,
        broker_account_id=broker_account_id, broker_route=broker_route,
        broker_route_generation=broker_route_generation, message=data.get("message"),
    )
...
class BrokerAdapter(ABC):
    ...
    def find_order_by_client_id(self, client_order_id: str):
        ...
        matches = [dict(o) for o in snapshot if str(o.get("client_order_id", o.get("tag", ""))) == client_order_id]
        if len(matches) > 1:
...
class UpstoxAdapter(BrokerAdapter):
    ...
    The client must expose explicit methods: place_order(payload), get_order(order_id),
    get_orders(), cancel_order(order_id), get_positions(), and get_profile(). No credentials
    are stored here. Order tags carry the platform client_order_id for authoritative recovery.
    """
    def __init__(self, client: Any, *, broker_account_id: str, broker_route_generation: str, broker_route: str = "upstox"):
        ...
        self.client = client
        self.broker_account_id = str(broker_account_id).strip()
        self.broker_route = str(broker_route).strip()
        self.broker_route_generation = str(broker_route_generation).strip()

    @staticmethod
    def _call(client, name: str, *args, **kwargs):
    ...
    def _normalize(self, raw: Any, *, expected: BrokerOrderRequest | None = None) -> BrokerOrderUpdate:
        ...
        if isinstance(data, list):
            ...
            data = dict(data[0])
        data["broker_account_id"] = self.broker_account_id
        data["broker_route"] = self.broker_route
        data["broker_route_generation"] = self.broker_route_generation
        data.setdefault("client_order_id", data.get("tag"))
        data.setdefault("symbol", data.get("trading_symbol") or (expected.symbol if expected else None))
        data.setdefault("side", data.get("transaction_type") or (expected.side if expected else None))
        return normalize_broker_update(data, expected=expected)

    def submit_order(self, order):
        raw = self._call(self.client, "place_order", self._payload(order))
        data = dict(self._data(raw) or {})
        order_id = str(data.get("order_id") or "").strip()
        if not order_id: raise RuntimeError("Upstox place_order response missing order_id")
        return self.get_order(order_id) if hasattr(self.client, "get_order") else self._normalize({**data, "status": "NEW", "order_id": order_id}, expected=order)

    def cancel_order(self, broker_order_id: str) -> BrokerOrderUpdate:
        raw = self._call(self.client, "cancel_order", str(broker_order_id))
        data = dict(self._data(raw) or {})
        data.setdefault("order_id", broker_order_id); data.setdefault("status", "CANCELLED")
        return self._normalize(data)

    def get_order(self, broker_order_id: str) -> dict[str, Any]:
        raw = self._call(self.client, "get_order", str(broker_order_id))
        data = dict(self._data(raw) or {})
    ...
    def get_orders(self) -> list[dict[str, Any]]:
        raw = self._call(self.client, "get_orders")
        data = self._data(raw)
    ...
    def get_positions(self) -> list[dict[str, Any]]:
        raw = self._call(self.client, "get_positions")
        data = self._data(raw)
    ...
    def get_account(self) -> dict[str, Any]:
        raw = self._call(self.client, "get_profile")
        data = dict(self._data(raw) or {})
        account_id = str(data.get("user_id") or data.get("userId") or "").strip()
    ...
    def health(self) -> BrokerHealth:
        try:
            ...
            return BrokerHealth(broker="upstox", healthy=True, authenticated=True, live_trading_enabled=True, message=f"authenticated account={account['broker_account_id']}")
