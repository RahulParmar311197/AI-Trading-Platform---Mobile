...
class PaperPortfolio:
    ...
    @property
    def equity(self):
        # Close P&L already includes close commission. Entry commission is
        # tracked separately so it is deducted exactly once.
    ...
    def close_position(
        ...
        if exit_price <= 0 or q <= 0 or q > p.quantity:
            raise ValueError("invalid close")
        pnl = p.unrealized_pnl(exit_price, q) - commission
        p.quantity -= q
        p.realized_pnl += pnl
    ...
    def partial_close(self, symbol: str, exit_price: float, quantity: float, move_stop_to_breakeven: bool = True, commission: float = 0.0, slippage: float = 0.0) -> CloseResult:
        ...
        if symbol in self.positions and move_stop_to_breakeven:
            self.positions[symbol].stop_loss = self.positions[symbol].entry_price
