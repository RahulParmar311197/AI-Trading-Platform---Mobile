...
class PaperPortfolio:
    ...
    def close_position(self,symbol:str,exit_price:float,reason:str='MANUAL',quantity:float|None=None)->CloseResult:
        ...
        if q<=0 or q>p.quantity: raise ValueError('invalid close quantity')
        pnl=p.unrealized_pnl(exit_price,q); p.quantity-=q; self.realized_pnl+=pnl
        if p.quantity==0: self.positions.pop(symbol)
        return CloseResult(symbol,p.side,p.entry_price,exit_price,q,pnl,reason)
    def partial_close(self,symbol:str,exit_price:float,quantity:float,move_stop_to_breakeven:bool=True)->CloseResult:
        result=self.close_position(symbol,exit_price,'PARTIAL_TP',quantity)
