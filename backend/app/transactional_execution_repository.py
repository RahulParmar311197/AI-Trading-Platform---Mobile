...
class TransactionalExecutionRepository:
    ...
    def _ensure_schema(self) -> None:
        ...
        if not position_columns:
            self._db.execute("CREATE TABLE positions(broker_account_id INTEGER NOT NULL,broker_route TEXT NOT NULL,symbol TEXT NOT NULL,quantity REAL NOT NULL,PRIMARY KEY(broker_account_id,broker_route,symbol))")
    ...
    def create_order(self, symbol: str, side: str, quantity: float, *, broker_account_id: int, broker_route: str) -> str:
        if quantity <= 0 or broker_account_id <= 0 or not broker_route:
            raise ValueError("positive quantity, broker account identity and broker route are required")
        order_id = str(uuid4())
        with self._lock:
            self._db.execute("INSERT INTO orders(order_id,symbol,side,quantity,status,broker_account_id,broker_route) VALUES(?,?,?,?,?,?,?)", (order_id,symbol.upper(),side.upper(),quantity,OrderStatus.CREATED.value,broker_account_id,broker_route))
            self._db.commit()
        return order_id
    ...
    def bind_identity(self, identity: OrderIdentity) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                if row and row != (identity.broker, identity.broker_order_id):
                ...
                if reverse and reverse[0] != identity.client_order_id:
                    raise ValueError("broker order is already bound to a different client order")
    ...
    def apply_event(self,event_id: str,order_id: str,kind: str,*,broker_account_id: int,broker_route: str,price: float|None=None,quantity: float=0.0)->bool:
        ...
        with self._lock:
            try:
                self._db.execute("BEGIN IMMEDIATE")
                if self._db.execute("SELECT 1 FROM execution_events WHERE event_id=?",(event_id,)).fetchone(): self._db.rollback(); return False
                row=self._db.execute("SELECT symbol,side,quantity,filled_quantity,status,broker_account_id,broker_route FROM orders WHERE order_id=?",(order_id,)).fetchone()
                if row is None: raise KeyError(order_id)
                symbol,side,total,filled,current,stored_account_id,stored_route=row
                if stored_account_id!=broker_account_id or stored_route!=broker_route: raise ValueError("broker account identity mismatch")
    ...
    def bind_identity_and_apply_event(self, identity: OrderIdentity, event_id: str, kind: str, *, broker_account_id: int, broker_route: str, price: float|None=None, quantity: float=0.0) -> bool:
        """Atomically bind an identity and apply an execution event in one DB transaction."""
        with self._lock:
            try:
                self._db.execute("BEGIN IMMEDIATE")
                row=self._db.execute("SELECT broker,broker_order_id FROM order_identity WHERE client_order_id=?",(identity.client_order_id,)).fetchone()
                if row and row!=(identity.broker,identity.broker_order_id): raise ValueError("client order identity conflict")
                reverse=self._db.execute("SELECT client_order_id FROM order_identity WHERE broker=? AND broker_order_id=?",(identity.broker,identity.broker_order_id)).fetchone()
                if reverse and reverse[0]!=identity.client_order_id: raise ValueError("broker order identity conflict")
                self._db.execute("INSERT OR IGNORE INTO order_identity(client_order_id,broker,broker_order_id) VALUES(?,?,?)",(identity.client_order_id,identity.broker,identity.broker_order_id))
                if self._db.execute("SELECT 1 FROM execution_events WHERE event_id=?",(event_id,)).fetchone(): self._db.rollback(); return False
                self._db.commit()
            except Exception:
                self._db.rollback(); raise
        return self.apply_event(event_id,identity.client_order_id,kind,broker_account_id=broker_account_id,broker_route=broker_route,price=price,quantity=quantity)
