...
class BrokerRouter:
    def __init__(self,routes:list[BrokerRoute],default_route:str,safety_store:SafetyStateStore|None=None,trading_gate:TradingGate|None=None,max_reconciliation_age_seconds:float=2.0,submission_intent_store:SubmissionIntentStore|None=None,reconciliation_engine:ReconciliationEngine|None=None,context_attestor:BrokerContextAttestor|None=None):
        ...
        if self.reconciliation_engine.submission_intent_store is not self.submission_intent_store: raise ValueError("reconciliation engine must use the router submission intent store")
    @contextmanager
    ...
    def reconcile_unresolved_submission_intents(self,route=None)->list[str]:
        ...
        with self._route_lifecycle_lock:
            ...
            for route_name, route_intents in grouped.items():
                selected=self.get(route_name if route is None else route); snapshot_fn=getattr(selected.adapter,"get_order_snapshot",None)
                ...
                for intent in route_intents:
                    ...
                    if len(matches)==1:
                        ...
                        actual_symbol=str(match.get("symbol") or "").strip().upper(); actual_side=str(match.get("side") or "").strip().upper(); actual_quantity=match.get("quantity",match.get("order_quantity",match.get("requested_quantity")))
    ...
    def _authoritative_reconciliation_snapshot(self,route:BrokerRoute)->BrokerSnapshot:
        self._verify_route_identity(route); order_snapshot_fn=getattr(route.adapter,"get_order_snapshot",None); position_snapshot_fn=getattr(route.adapter,"get_position_snapshot",None)
    ...
    def _next_reconciliation_generation(self, route: BrokerRoute) -> int:
        ...
        try:previous=int(raw)
        except (TypeError,ValueError):raise RuntimeError("persisted broker reconciliation generation is invalid")
        if previous<0:raise RuntimeError("persisted broker reconciliation generation is invalid")
        return previous+1
        ...
        with self._route_lifecycle_lock:
            ...
            try:
                if self.safety_store is not None:
                    ...
                    record=self._reconciliation_record(selected);expected=record.get("broker_snapshot_fingerprint") if record else None
