...
def _persisted_local_positions(lifecycle: OrderLifecycle) -> dict[str, float]:
    ...
    for symbol, position in lifecycle.positions.items():
        ...
        positions[str(symbol).strip().upper()] = positions.get(str(symbol).strip().upper(), 0.0) + signed
...
async def lifespan(app:FastAPI):
    ...
    startup_recovery.state=RecoveryState.READY if result.ready else RecoveryState.FAILED
    ...
    try:
        broker_positions=broker_router.get_positions()
    except Exception:
