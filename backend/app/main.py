...
async def lifespan(app: FastAPI):
    ...
    if recovery_route is None:
        trading_health.record("broker_recovery", True, "no active broker accounts")
        startup_state.transition(StartupExecutionState.BROKER_RECONCILED, "no active broker accounts")
        startup_state.transition(StartupExecutionState.PORTFOLIO_RECONCILED, "no active broker accounts")
        startup_state.transition(StartupExecutionState.RISK_READY, "no active broker accounts")
        startup_state.transition(StartupExecutionState.READY)
    elif len(active_accounts) > 1:
        multi_recovery = MultiAccountStartupRecovery(execution_broker_router, execution_store, safety_store, recovery_manager)
        multi_result = multi_recovery.run(lifecycle, active_accounts)
        app.state.multi_account_recovery_result = multi_result
        trading_health.record("multi_account_recovery", multi_result.ready, "ready" if multi_result.ready else "failed")
        if not multi_result.ready:
            startup_state.fail("multi-account broker startup recovery failed")
        else:
...
for router in [auth_router, broker_accounts_router, upstox_oauth_router, emergency_halt_router, health_router, execution_health_router, orders_router, backtest_router, create_operational_router(trading_health, trading_metrics), create_risk_circuit_router(risk_circuit_breaker)]:
