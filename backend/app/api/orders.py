...
def _execution_service(broker_router,execution_store,idempotency_store,recovery,db,user_id):
    ...
    gate=PreTradeRiskGate(RiskLimits(settings.risk_max_order_quantity,settings.risk_max_position_quantity,settings.risk_max_daily_loss,settings.risk_max_trade_loss))
    ...
    return OrderExecutionService(broker_router,lifecycle,execution_store,idempotency_store,recovery=recovery,risk_gate=gate,risk_snapshot_provider=provider)

def _broker_request(client_order_id,symbol,side,quantity,order_type="MARKET",price=None,stop=None,security_id=""):
    return BrokerOrderRequest(client_order_id=client_order_id,symbol=symbol,side=side,quantity=quantity,order_type=order_type,price=price,stop=stop,security_id=security_id)
...
def create_order(payload:OrderRequest,request:Request,response:Response,db:Session=Depends(get_order_db),_:None=Depends(require_trading_ready),idempotency_key:str|None=Header(default=None,alias="Idempotency-Key")):
    ...
    if existing is not None:
        if existing.note=="EXECUTION_PENDING_RECONCILIATION":
            service=_execution_service(broker_router,execution_store,idempotency_store,recovery,db,existing.user_id)
            result=service.submit(_broker_request(client_order_id,existing.symbol,existing.side,existing.quantity,existing.order_type))
            existing.status=result.status; existing.broker_order_id=result.broker_order_id; existing.note=result.message; db.commit(); db.refresh(existing); _set_execution_response_status(response,result.status); return _order_response(existing,result.message)
    ...
    try: db.flush()
        ...
        return _order_response(existing,"IDEMPOTENT_REPLAY")
    service=_execution_service(broker_router,execution_store,idempotency_store,recovery,db,payload.user_id)
    result=service.submit(_broker_request(client_order_id,symbol,payload.side,payload.quantity,payload.order_type,payload.price,payload.stop,payload.security_id))
    order.status=result.status; order.broker_order_id=result.broker_order_id; order.note=result.message; db.commit(); db.refresh(order); _set_execution_response_status(response,result.status); return _order_response(order,result.message)
