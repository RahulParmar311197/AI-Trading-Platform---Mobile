...
def _order_response(order:Order,message:str|None=None)->dict:return {"id":order.id,"client_order_id":order.client_order_id,"broker_order_id":order.broker_order_id,"status":order.status,"message":message}
...
def create_order(payload:OrderRequest,request:Request,response:Response,db:Session=Depends(get_order_db),_:None=Depends(require_trading_ready),idempotency_key:str|None=Header(default=None,alias="Idempotency-Key")):
    ...
    resources=getattr(request.app.state,"resources",None)
    if resources is not None: broker_router=request.app.state.broker_router; execution_store=resources.execution_store; idempotency_store=resources.idempotency_store
    recovery=getattr(request.app.state,"startup_recovery",None) or StartupRecoveryCoordinator(); client_order_id=idempotency_key.strip() if idempotency_key else str(uuid.uuid4())
    ...
    if existing is not None:
        if existing.note=="EXECUTION_PENDING_RECONCILIATION":
            service=_execution_service(broker_router,execution_store,idempotency_store,recovery,resources); result=service.submit(_broker_request(client_order_id,existing.symbol,existing.side,existing.quantity,existing.order_type)); existing.status=result.status; existing.broker_order_id=result.broker_order_id; existing.note=result.message; db.commit(); db.refresh(existing); _set_execution_response_status(response,result.status); return _order_response(existing,result.message)
