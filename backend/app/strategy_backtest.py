...
def run_strategy_backtest(
    ...
    if len(candles) < 40:
        raise ValueError("at least 40 candles required")
    ...
    for i in range(35, len(candles)):
        ...
        if position:
            ...
            hit_target = (
                ...
                if direction == 1
                else candle.low <= position["target"]
            )
        ...
        result = decide(
            candles[max(0, i - 200): i + 1]
        )

        if (
            result.action == "NO_TRADE"
            or result.confidence < config.min_confidence
        ):
            continue
    ...
    return {
        ...
        "trades": trades,
        "metrics": trading_metrics(returns),
