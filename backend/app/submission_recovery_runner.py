...
class SubmissionRecoveryRunner:
    """Bounded, framework-independent recovery entrypoint; never retries indefinitely."""

    def __init__(self, orchestrator: OrderSubmissionRecoveryOrchestrator) -> None:
        self.orchestrator = orchestrator

    def run_once(self, *, limit: int = 100) -> RecoveryRun:
        if limit <= 0 or limit > 1000:
            raise ValueError("limit must be between 1 and 1000")
        started = monotonic()
        results = self.orchestrator.recover_pending(limit=limit)
